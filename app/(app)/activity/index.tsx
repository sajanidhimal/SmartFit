...
import { logSteps } from '../../utils/database_service/exercise_tracking_functions';
...
export default function StepTrackerApp() {
  ...
  const startTracking = async () => {
    try {
      ...
      // Save initial step count when starting
  ...
  const stopTracking = () => {
    ...
    // Save steps to Firestore when stopping tracking
    const stepsToLog = steps - initialSteps;
    const userId = auth.currentUser?.uid;
    
    if (userId && stepsToLog > 0) {
      logSteps(userId, {
        steps: stepsToLog,
        date: Timestamp.now()
      })
      .then(result => {
        if (result.success) {
          Alert.alert("Step Tracking", `Stopped tracking. Saved ${stepsToLog} steps to your profile!`);
        } else {
          Alert.alert("Error", "Stopped tracking but failed to save steps to your profile.");
        }
      })
      .catch(error => {
        console.error("Error saving steps to Firestore:", error);
        Alert.alert("Error", "Failed to save steps to your profile.");
      });
    } else {
